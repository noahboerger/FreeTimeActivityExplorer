...
public abstract class BaseDao<E, I, K> implements Serializable, BasicDataAccess<E, K> {
	...
	/**
	 * This method can be overridden in sub classes to alter an entity before a save
	 * operation (persist or merge).
	 *
	...
	/**
	 * Tries to determine whether the entity is new or already in the DB by looking
	 * at its Id key.
	 *
	 * @param entity
	 */
	...
	/**
	 * Returns a list sorted by the given function. The list will be sorted by Java,
	 * so this is an expensive operation for big lists.
	 *
	 * @param stringFieldExtractor extractor function, e.g.
	...
	/**
	 * Tries to find a managed entity corresponding to the given (possibly detached)
	 * entity. If the entity is already managed, it will be returned immediately
	 * without Db lookup, so it is safe to call this function. <strong> Please be
	 * aware that any modifications to the given entity will not be reflected by the
	 * returned entity, if it has to be re-loaded from the persistence
	 * context!</strong> If the entity is not persisted nor has a valid key, null
	 * will be returned.
	 *
	 * @param e entity
